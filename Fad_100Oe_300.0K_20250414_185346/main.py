...
#         timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
#         self.data_file = tk.StringVar(value=f"{self.material}_{timestamp}.dat")
#         self.plot_file = tk.StringVar(value=f"{self.material}_{timestamp}.png")
#
#         ttk.Label(frame, text="QD Server IP:").grid(row=0, column=0)
#         ttk.Entry(frame, textvariable=self.host_ip).grid(row=0, column=1)
#         ttk.Label(frame, text="Folder:").grid(row=1, column=0)
...
class MeasurementGUI:
    ...
    def create_host_output_frame(self):
        ...
        # Initialize filename StringVars first
        ...
        ttk.Label(frame, text="Material:").grid(row=0, column=0)
        ttk.Entry(frame, textvariable=self.material).grid(row=0, column=1)
        ttk.Label(frame, text="QD Server IP:").grid(row=1, column=0)
        ttk.Entry(frame, textvariable=self.host_ip).grid(row=1, column=1)
        ttk.Label(frame, text="Folder:").grid(row=2, column=0)
        ttk.Entry(frame, textvariable=self.folder_path).grid(row=2, column=1)
        ttk.Button(frame, text="Browse", command=self.browse_folder).grid(row=2, column=2)
        ttk.Label(frame, text="Data File:").grid(row=3, column=0)
        ttk.Entry(frame, textvariable=self.data_file).grid(row=3, column=1)
        ttk.Label(frame, text="Plot File:").grid(row=4, column=0)
        ttk.Entry(frame, textvariable=self.plot_file).grid(row=4, column=1)

        # Update filenames when material changes
        self.material.trace_add("write", lambda *args: self.update_filenames())

    def update_filenames(self):
        """Update filenames based on current material and timestamp"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        material = self.material.get()
        # Update the existing StringVars instead of creating new ones
        self.data_file.set(f"{material}_{timestamp}.dat")
        self.plot_file.set(f"{material}_{timestamp}.png")
    ...
    def run_measurement(self):
        ...
        try:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            material = self.material.get()
            session_folder = os.path.join(folder, f"{material}_{max_field}Oe_{temp}K_{timestamp}")
            os.makedirs(session_folder, exist_ok=True)

            # Set up file paths
            datafile = os.path.join(session_folder, f"{material}_data_{timestamp}.dat")
            plotfile = os.path.join(session_folder, f"{material}_plot_{timestamp}.png")
            logfile = os.path.join(session_folder, f"{material}_log_{timestamp}.txt")
